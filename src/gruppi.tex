...
\end{proposition}
\begin{proof}
    $C \tri G \iff \forall \varphi \in \Inn(G) \ \varphi(C) = C$ ma $N \tri G \Rightarrow \forall F \in \Inn(G) \ F\mid_N \in \Aut(N)$ e $C$ caratteristico in $N \Rightarrow \forall f \in \Aut(N) \ f(C)= C$. Mettendo assieme le due cose: $ \forall \varphi \in \Inn(G) \ \varphi(C)  = \varphi\mid_H(C) = C$. 
\end{proof}
...
\begin{proof}
    ...
    \end{align*}
    Usando che \hyperlink{es1}{$G$ non è unione di sottogruppi coniugati} (dimostrato nel paragrafo $1.7$) si ha che un tale $g$ esiste sempre se $\text{stab}(x_0) \neq G$. Ma $\text{stab}(x_0) = G$ e $\text{orb}(x_0) = X$ (azione transitiva) dà un assurdo per il lemma orbita-stabilizzatore, quindi $\text{stab}(x_0) \neq G$ da cui la tesi. 
\end{proof}
...
\begin{proof}
    ...
    \end{itemize}
    Notiamo ora che: $\alpha \circ \beta$ manda $\overline H \mapsto  \pi_N(\pi_N^{-1}(\overline H)) = \overline H$ dove l'ultima uguaglianza segue dal fatto che $\pi_N$ è suriettiva. Invece $ \beta \circ \alpha$ manda 
    $H \mapsto  \pi_N^{-1}(\pi_N(H)) = \pi_N^{-1}(H/N) = \{g \in G : gN \in H/N \} =  H$
...
\begin{definition}{prodotto semidiretto}
    Siano $H, K$ due gruppi, e $\phi: K \rightarrow \Aut(H)$ un omomorfismo. Indichiamo con $\phi_g$ l'immagine di $g\in K$ secondo $\phi$ e definiamo l'operazione $*$ sul prodotto cartesiano $H \times K$ data da $(h_1,k_1)*(h_2,k_2) = (h_1\phi_{k_1}(h_2), k_1k_2)$. Indichiamo con $H \rtimes_{\phi} K$ questo nuovo gruppo, che chiamiamo un \textit{prodotto semidiretto} di $H$ e $K$.
\end{definition}

Si noti che se $\phi \equiv id_{K}$ si ottiene il prodotto diretto (e viceversa). D'ora in poi il segno ``$*$'' verrà omesso, come le altre operazioni.
...
\end{proposition}
\begin{proof}
    Consideriamo $F: H \rtimes_{\varphi} K \to  H \rtimes_{\psi} K$ tale che $F((h,k) \mapsto (\alpha(h), \beta(k))$.
    \begin{itemize}
        \item è omomorfismo: $F((h,k)*_{\varphi}(h',k')) = F(h\varphi_{k}(h'),kk') =  (\alpha(h)\alpha\circ\varphi_{k}(h'),\beta(k)\beta(k')) =$
        
        $= (\alpha(h)\psi_{\beta(k)}(\alpha(h')),\beta(k)\beta(k')) =(\alpha(h), \beta(k))*_{\psi} (\alpha(h'),\beta(k')) =  F((h,k))*_{\psi}F((h',k'))$
...
\begin{proof}
    ...
    \begin{align}
    \ker(\pi|_H) & = \{h \in H : \pi(h) =\overline 0 \} =  \nonumber \\
    ...
    \begin{itemize}
    \item $\langle x_1 \rangle + H  = G$: $\forall x \in G$ vale che $\pi(x) = a_2\overline x_2 + \dots + a_t \overline x_t \Rightarrow \pi(x - (a_2 x_2 + \dots + a_t  x_t)) = \overline0 \Rightarrow x - (a_2 x_2 + \dots + a_t  x_t) \in \langle x_1 \rangle$;
...
\begin{proposition}{proprietà del sottogruppo derivato}
    ...
    \begin{enumerate}
        ...
        \begin{proof}
            ...
            \item $\varphi \in \Hom(G/G',H)$ la mandiamo in $\varphi \circ \pi \in \Hom(G',H)$ dove $\pi$ è la proiezione $\pi: G \rightarrow G/G'$. Anche questa mappa è chiaramente iniettiva, visto che $G' \subseteq \ker(\varphi)$ e quindi se due mappe vengono mandate nella stessa allora coincidono anche su tutto $G$.
            \end{itemize}
        \end{proof}
        \end{enumerate}    
\end{proposition}
...
\begin{proposition}{prodotti diretti belli}
    Se $G = H \times K$ con $H,K$ finiti tali che $(\#H, \#K) = 1$ allora $\{e_H\} \times K$ e $H \times \{e_K\}$ sono caratteristici in $G$.
\end{proposition}
\begin{proof}
...
\begin{proposition}{automorfismi in un prodotto diretto}
    Se $G = H \times K$ e $\{e_H\} \times K$ e $H \times \{e_K\}$ sono caratteristici in $G$, allora $\Aut(G) \cong \Aut(H) \times \Aut(K)$. 
\end{proposition}
\begin{proof}
...
\begin{proposition}{(*) centro di un prodotto semidiretto}
    Sia $G= H \rtimes_{\varphi} K$ con $H$ abeliano. Allora
    \[
    Z(G) = \left( \bigcap_{k \in K} \Fix(\varphi_k) \right) \times (\ker(\varphi) \cap Z(K)).
    \]
\end{proposition}
\begin{proof}
...
\begin{proof}
    Sia $H_i = \langle r^{d_i}, sr^{h_i} \rangle$ per $i = 1,2$. Notiamo che $H_1 = H_2 \Rightarrow \grp{r^{d_1}} = H_1 \cap R = H_2 \cap R = \grp{r^{d_2}}$. Per quanto osservato prima, ciò implica $d_1 = d_2$. Si può ora notare che $H_i = \grp{sr^{h_i}}\grp{r^{d_i}}$ (basta osservare che $r^{d_i}sr^{h_i}=sr^{h_i-d} \in \grp{sr^{h_i}}\grp{r^{d_i}}$). Allora $sr^{h_2} \in H_1  \Rightarrow sr^{h_2} = (sr^{h_1})^a(r^{d_1})^b$ da cui segue che $a$ è dispari (o il membro di destra non sarebbe una simmetria). Ma allora $(sr^{h_1})^a(r^{d_1})^b=sr^{h_1+bd_1}$ e quindi $h_1 + b d_1  \equiv h_2 \pmod{n}$. In particolare $h_1 \equiv h_2 \pmod{d_1}$. Ma $d_1 = d_2$ e l'ipotesi implicano che sono entrambi ridotti modulo $d_1$ e quindi non si ha solo congruenza ma uguaglianza.
\end{proof}
...
\begin{proof}
    ...
    Sia $\sigma ' = (b^1_1, \dots, b^1_{k_1})(b^2_1, \dots, b^2_{k_2}) \dots (b^c_1, \dots, b^c_{k_c}) \in X$; notiamo che $\tau = (a^1_1,b^1_1)\circ\dots \circ (a^c_{k_c},b^c_{k_c})$ è tale che $\tau\circ \sigma\circ \tau^{-1} = \sigma'$. Quindi $\sigma' \in Cl(\sigma)$. Facendo variare $\sigma'$, $X \subseteq Cl(\sigma)$. 
\end{proof}
...
\begin{proposition}{cardinalità di un centralizzatore}
    ...
    Ciò ci può aiutare a capire come sono fatti dei centralizzatori semplici oppure, combinato con il lemma normalizzatore-centralizzatore, dei normalizzatori.
...
\begin{proof}
    ...
    \begin{lemma2}
        Sia $\sigma \in A_n\backslash\{e\}$ con $n \ge 6$. Allora $\exists i \in \{1, \dots, n\}, \tilde \sigma \in A_n$ coniugata non banale di $\sigma$ tale che $\sigma(i) = \tilde \sigma(i)$.
    \end{lemma2}
    \begin{proof}
...
\begin{proposition}{proprietà universale del gruppo libero}
    per ogni gruppo $G$ si ha \[\Hom(F_n, G) \leftrightarrow G^n \cong \{(g_1, \dots, g_n) : g_i \in G \ \forall 1 \leq i \leq n\}, \] ovvero sono in bigezione.
\end{proposition}
\begin{proof}
...
\end{example}
\begin{example}
    Per dire che due elementi commutano basta scrivere $[a,b] = 1$ (si ricorda che $[a,b] = aba^{-1}b^{-1}$ è il commutatore). Quindi ad esempio $\Z/2\Z \times \Z/2\Z = \grp{x,y \mid x^2 = y^2 = xyx^{-1}y^{-1} = e}$ e l'informazione "il gruppo è abeliano" può essere espressa mediante le relazioni $\set{[g,h] = 1 \mid g,h \in G}$.
\end{example}
...
\begin{proof}
    ...
    Guardando le possibilità e le cardinalità, necessariamente uno tra $P_2$ e $P_3$ è normale in $G$ { \tiny (due $3$-Sylow distinti devono necessariamente avere intersezione banale, quindi se $n_3 = 4$ si ha che i $3$-Sylow hanno unione di cardinalità $4\cdot (3-1) + 1 = 9$; un 2-Sylow ha chiaramente intersezione banale con questa unione, quindi deve essere contenuto nei $12 - 9 + 1 = 4$ elementi rimanenti, da cui segue che ce ne può essere uno solo) }, quindi $G \cong P_2 \rtimes P_3$ oppure $G \cong P_3 \rtimes P_2$. Poiché $P_3 \cong \Z / 3 \Z$ e $P_2 \cong \Z / 4 \Z$ oppure $P_2 \cong \Z / 2\Z \times \Z / 2\Z$ facciamo i casi: 
    \begin{itemize}
        \item $G \cong P_2 \rtimes P_3$, $P_2 \cong \Z / 4 \Z$. Quindi $G \cong  \Z / 4 \Z \rtimes_{\varphi} \Z / 3 \Z$. $\varphi : \  \Z / 3 \Z \rightarrow \text{Aut}(\Z / 4 \Z) \cong \Z / 2 \Z$ e quindi l'unica scelta per $\varphi$ è l'omomorfismo banale (i due gruppi hanno ordini coprimi). Quindi  $G \cong \Z / 4 \Z \times \Z / 3 \Z \cong \Z / 12 \Z$.
        ...
        \begin{itemize}
            \item $\varphi \equiv id$ e quindi  $G \cong (\Z / 2 \Z)^2 \times \Z / 3 \Z \cong \Z / 2 \Z \times \Z / 6 \Z$.
            \item $\varphi \not\equiv id$ che ci dà $G \cong A_4$, con l'isomorfismo che manda $(\Z / 2 \Z)^2$ nel Klein e $\Z / 3 \Z$ nel gruppo generato da un tre ciclo.
        \end{itemize} 
        ...
        \begin{itemize}
            \item $\varphi \equiv id$ e quindi  $G \cong \Z / 4 \Z \times \Z / 3 \Z \cong \Z / 12 \Z$ (già visto)
            \item $\varphi \not\equiv id$ che non dà "gruppi noti"; scriviamo solo $G \cong \Z/3\rtimes \Z/4\Z$
        \end{itemize} 
        \item $G \cong P_3 \rtimes P_2$, $P_2 \cong \Z / 2 \Z \times \Z/2\Z$. Quindi $G \cong \Z / 3 \Z  \rtimes_{\varphi} (\Z/2\Z)^2$. $\varphi : \  (\Z/2\Z)^2  \rightarrow \text{Aut}(\Z / 3 \Z) \cong \Z / 2 \Z$ e quindi abbiamo due scelte (in realtà le scelte sono 4, se consideriamo i due generatori canonici, ma tre di esse sono chiaramente isomorfe):
        \begin{itemize}
            \item $\varphi \equiv id$ e quindi $G \cong (\Z / 2 \Z)^2 \times \Z / 3 \Z \cong  \Z / 2 \Z \times \Z / 6 \Z$ già visto. 
            \item $\varphi \not\equiv id$, e quindi $\ker(\varphi) \cong \Z/2\Z$. Sia $\ker(\varphi) = \grp{x}$ con $x \in G$, e siano $y,z \in G$ tali che $\grp{z} \cong \Z/3\Z$ e $\grp{x}\grp{y} \cong (\Z / 2 \Z)^2 \Rightarrow G \cong \grp{z} \rtimes \grp{x}\grp{y}$. Allora per definizione (essendo $\varphi_z$ il coniugio per $z$) si ha $zxz^{-1} = x, zyz^{-1} = xy = yx \Rightarrow $
        Si può allora mostrare che $\grp{x}\grp{z} \cong \Z/6\Z$, poiché ha ordine $6$ ed è abeliano. Quindi $\grp{x}\grp{z}=\grp{r}$ per un $r\in G$ tale che $x=r^3 \text{ e }z=r^2$. Sostituendo nell'uguaglianza precedente si ottiene $yr^{-1}=ry$ e $G\equiv D_6$.
        \end{itemize} 
    \end{itemize}
\end{proof}
...
\begin{proof}
    ...
    $(\ell,15) = 1$, perché altrimenti $\varphi_y$ non sarebbe automorfismo, e che $x = \varphi_y(\varphi_y(x)) = x^{\ell^2} \Rightarrow \ell^2 \equiv 1 \pmod{15}$. Ciò implica
    $\begin{cases}
    ...
    \begin{enumerate}
        ...
        Si può osservare che ciò implica $yx^5y^{-1} = x^5  \Rightarrow x^5 \in Z(G)$. 
        ...
        In modo analogo a prima $x^3 \in Z(G)$ e si verifica manualmente $G \cong D_3 \times \Z/5\Z$.
