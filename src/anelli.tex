...
\begin{theorem}{ideali e omomorfismi}
    ...
    \begin{itemize}
        ...
        \item se $M\supseteq \mathrm{Ker}(f)$ immagine di ideale massimale $M$ è ideale massimale.
    \end{itemize}
\end{theorem}
\begin{proof}
    esercizio.
...
\end{proposition}
\begin{proof}
    Per monotonia $\sqrt{IJ} \subseteq \sqrt{I \cap J}$. D'altra parte, $x^n \in I \cap J \implies x^{2n} = x^n x^n \in IJ$, dunque $\sqrt{IJ} = \sqrt{I \cap J}$. Ancora per monotonia si ha $\sqrt{IJ} \subseteq \sqrt{I} \cap \sqrt{J}$, inoltre dato $x$ tale che $x^n \in I$ e $x^m \in J$ si ha $x^{n+m} \in IJ$, quindi l'ultima uguaglianza.
\end{proof}
...
\begin{proposition}{$A$ si immerge nella localizzazione}
    la mappa $f: A \hookrightarrow S^{-1}A$ data da $f(a \mapsto \frac{a}{1})$ è un omomorfismo di anelli, inoltre, se $A$ è un dominio, è iniettiva (la verifica è immediata).
\end{proposition}
...
\end{proposition}
\begin{proof}
    Un elemento $\frac{a}{s}$ della localizzazione è invertibile sse $\exists \frac{b}{t} \in S^{-1}A$ tale che $\frac{a}{s} \cdot \frac{b}{t} = \frac{ab}{st} = \frac{1}{1}$, cioè $uab = ust$, che accade se e soltanto se esiste in $S$ un multiplo di $a$.
\end{proof}
...
\end{theorem}
\begin{proof}
    Se $I$ è un ideale di $A$ allora $S^{-1}I = \{ \frac{x}{s} : x \in I,s\in S\}$ è un ideale di $S^{-1}A$. La proprietà di assorbimento di $S^{-1}I$ segue da quella di $I$ e da $S$ parte moltiplicativa. Inoltre $S^{-1}I$ è un sottogruppo additivo, infatti:
    \begin{itemize}
...
\end{theorem}
\begin{proof}
    Dimostriamo innanzitutto che se $P$ è un ideale primo di $A$ disgiunto da $S$, allora $S^{-1}P$ è un ideale primo di $S^{-1}A$. Si nota intanto $S^{-1}P \neq S^{-1}A$, infatti $S^{-1}P = S^{-1}A \iff \frac{1}{1} \in S^{-1}P \iff S \cap P \neq \emptyset$. Inoltre $S^{-1}P$ è un ideale per il teorema precedente. Mostriamo che è primo: dato $\frac{x}{s} \frac{y}{t} = \frac{xy}{st} = \frac{p}{r} \in S^{-1}P$ si ha $uxyr = upst \in P$ per assorbimento, dunque per primalità vale almeno una tra $x \in P$, $y \in P$ e $ur \in P$. Sappiamo $ur \notin P$ perché $ur \in S$ e $S \cap P = \emptyset$, segue $x \in P \lor y \in P$ e quindi $\frac{x}{s} \in S^{-1}P \lor \frac{y}{t} \in S^{-1}P$.
    ...
    $\alpha \circ \beta = id$ per quanto visto nel teorema precedente. Mostriamo anche $\beta \circ \alpha = id$, ossia $\forall P \in \mathcal{P} \ P = f^{-1}(S^{-1}P)$. Il contenimento $P \subseteq f^{-1}(S^{-1}P)$ segue da $x \in P \Rightarrow \frac{x}{1} \in S^{-1}P \Rightarrow x \in f^{-1}(S^{-1}P)$. Per quello inverso osserviamo che $x \in f^{-1}(S^{-1}P) \Rightarrow  \ \frac{x}{1} = \frac{p}{r} \in S^{-1}P$ con $p\in P, r \in S$, quindi $uxr = up \in P$ e, poiché $r \in S$ e $S \cap P = \emptyset$, necessariamente $x \in P$. Quindi $f^{-1}(S^{-1}P) \subseteq P$.
\end{proof}
...
\begin{proof}
    ...
    $P$ è primo $\iff \forall x,y \in A \ (xy \in P \Rightarrow x \in P$ o $y \in P) \iff \forall x,y \in A \ (x \not \in P \text{ e } y \not \in P \Rightarrow xy \not \in P) \iff \forall x,y \in A \ (x \in S \text{ e } y \in S \Rightarrow xy \in S) \iff S$ è un semigruppo moltiplicativo, quindi sse $S$ è una parte moltiplicativa. 
\end{proof}
\begin{definition}{localizzazione}
...
\begin{definition}{massimo comune divisore}
    Siano $a, b \in A$ non entrambi nulli. Un elemento $d \in A$ si dice ``massimo comune divisore'' $MCD(a, b)$ di $a$ e $b$ se per ogni $c$ vale l'implicazione $c \mid a \land c \mid b \iff c \mid d$. Si noti che l'MCD è definito a meno di associati.
...
\begin{proof}
    ...
    Sia $A$ PID; per quanto visto prima $A$ è anche UFD. Consideriamo $d = MCD(a,b)$ secondo la definizione negli UFD. Dobbiamo dimostrare che vale anche $(d) = (a,b)$ e quindi $d$ coincide con quello trovato dalla definizione per i PID. Guardiamo la fattorizzazione degli elementi in $(a,b) = \{ax+by : x,y \in A\}$. Possiamo sicuramente isolare dai due addendi $\prod_{i=1}^s p_i^{\min\{\alpha_i,\beta_i\}}$, da cui segue che $(a,b) \subseteq (d)$. Per l'altro contenimento, osserviamo che $A$ PID $\Rightarrow \exists d' \in A \ (a,b) = (d') \subseteq (d) \Rightarrow \prod_{i=1}^s p_i^{\min\{\alpha_i,\beta_i\}}\mid d'$. Se però almeno uno degli esponenti fosse più grande, diciamo quello di $p_1$ avrei un assurdo perché, supponendo senza perdità di generalità $\alpha_i\leq \beta_i$, $p_1^{\min(\alpha_1,\beta_1)+1}\mid d'\mid a$ che è assurdo.
    
    Sia ora $A$ ED; per quanto visto prima $A$ è anche PID. Sia $d$ tale che $(d) = (a,b)$. Induciamo sul numero di passi dell'algoritmo di Euclide. Se l'algoritmo termina subito (cioè se $a = qb$), allora è chiaro $(b) = (a, b) = (d)$. Se invece $a = qb + r$ con $r \neq 0$ sia $\tilde d$ l'elemento trovato dall'algoritmo applicato a $b$ e $r$, per ipotesi induttiva $(\tilde d) = (b, r)$. Per ogni $x$ vale $(x \mid b \land x \mid r) \iff (x \mid b \land x \mid a)$. Allora $\tilde d \mid a \land \tilde d \mid b$, da cui $\tilde d \mid d$, ma anche $d \mid b \land d \mid r$, da cui $d \mid \tilde d$, dunque $(d) = (\tilde d)$.
\end{proof}
...
\begin{proposition}{(*) MCD ed estensioni}
    Siano $A \subset B$ due UFD e $(a, b) \in A$. Indichiamo con $d_A = MCD_A(a, b)$ il massimo comun divisore di $a$ e $b$ su $A$, con $d_B$ quello su $B$. Vale sempre $d_A \mid_B d_B$. Se inoltre su $A$ vale Bezout, allora anche $d_B \mid_B d_A$, ma l'ultima divisibilità è falsa in generale.
\end{proposition}
\begin{proof}
...
\begin{proof}
    ...
    \begin{proof}
        sia $f(x) \in \sqrt{(0)}[x]$. Poiché $f(x)$ ha finiti coefficienti tutti nilpotenti, esiste un esponente $M$ tale che tutti i coefficienti elevati a quel numero diano $0$. Allora sviluppando con il multinomio di Newton si ha che $f(x)$ elevato alla $(\text{deg}(f)+1)M$ fa il polinomio nullo, da cui segue $f(x) \in \sqrt{(0)[x]}$. 
    \end{proof}
...
\end{corollary}
\begin{proof}
    Come prima siano $a\in A, g_0 \in A[x]$ tali che $ag(x) = g_0(x)$. Sia $g_0(x) = c(g_0)g_1(x)$ con $g_1(x)$ primitivo. Allora $g_1(x) \mid f(x)$  in $K[x]$ e siamo nelle ipotesi del corollario 1, e quindi $g_1(x) \mid f(x) \in A[x]$. Da $af(x) = ag(x)h(x) = c(g_0)g_1(x)h(x)$ segue che allora possiamo definire $h_1(x) := a^{-1}c(g_0)h(x) \in A[x]$ e abbiamo la tesi.
\end{proof}
